    ...
    function walk(fileDir, rule) {
        ...
        util.forEach(files, function (fileName) {
            ...
            if (lstat.isDirectory()) {
                if (fileName === 'js') {
                    util.forEach(fs.readdirSync(baseDir), function (fileName) {
                    ...
                    notEmpty(opt.rjs) && util.forEach(fs.readdirSync(baseDir), function (fileName) {
                        if (/^.+\.js$/.test(fileName)) {
                            fileManipulation(fileName, baseDir + '/' + fileName, 'rjs');
                        }
                    });
                } else if (fileName === 'css' && rule.indexOf('css') != -1) {
                    notEmpty(opt.css) && util.forEach(fs.readdirSync(baseDir), function (fileName) {
                        if (/^.+\.css$/.test(fileName)) {
                            fileManipulation(fileName, baseDir + '/' + fileName, 'css');
                        }
                    });
                } else if (fileName === 'scss') {
                    notEmpty(opt.scss) && util.forEach(fs.readdirSync(baseDir), function (fileName) {
