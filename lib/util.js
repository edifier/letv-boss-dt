...
const util = {
    ...
    relative: (basePath = '', outPath = '') => {
        let symbol = PATH.sep, dirArr = outPath.replace(/(\\|\/)/g, symbol).split(symbol), $p;
    ...
    isInDirectory: (...arg) => {
        let dirPath = PATH.normalize(PATH.resolve(arg[1])),
            filePath = PATH.normalize(PATH.resolve(arg[0])).split(PATH.sep);

        let is = true;

        util.forEach(dirPath.split(PATH.sep), function (dir, i) {
            if (dir != filePath[i]) {
                is = false;
                return false;
            }
     ...
     * return boolean;
     */
    testRJS: (file = '') => /.+\/rjs\/\w+\.js/gi.test(file.replace(/\\/gi, '/')) || /.+_rjs\.js/gi.test(PATH.basename(file)),
